...
function sizeBudget(n: number): number[] {
  ...
  for (let i = 0; i < b2; i++) sizes.push(nth(mid, i % 3))
  const top = [1.38, 1.5, 1.38]
  for (let i = 0; i < b3; i++) sizes.push(nth(top, i % 3))
...
function bestSpot(placed: Disc[], size: number, gap: number, wallR: number) {
  ...
  for (let j = 0; j < placed.length; j++) {
    for (let k = j + 1; k < placed.length; k++) {
      const A = nth(placed, j)
      const B = nth(placed, k)
...
function solveRimGap(sizes: number[], R: number, gap: number): number {
  const total = (g: number) => {
    ...
    for (let i = 0; i < sizes.length; i++) {
      const t = rimStep(R, nth(sizes, i), nth(sizes, (i + 1) % sizes.length), g)
...
function placeRim(sizes: number[], R: number, gap: number): Disc[] {
  ...
  for (let i = 0; i < sizes.length; i++) {
    ...
    const r = R - z / 2 - wob
    discs.push({
      x: round(Math.cos(th) * r),
      y: round(Math.sin(th) * r),
      size: z,
    })
    th += rimStep(R, z, nth(sizes, (i + 1) % sizes.length), gap)
  }
  return discs
}
  ...
  for (let sweep = 0; sweep < 60; sweep++) {
    ...
    for (const i of order) {
      ...
      for (let j = 0; j < out.length; j++) {
        if (j === i) continue
        const need = (d.size + nth(out, j).size) / 2 + g - 0.5
        if (Math.hypot(nx - nth(out, j).x, ny - nth(out, j).y) < need) {
          ok = false
          break
        }
      }
...
function relaxToFill(
  discs: Disc[],
  nRim: number,
  R: number,
  g: number,
): { discs: Disc[]; gi: number } {
  let settled = discs
  let lo = g
  let hi = g * 3
  ...
  return {
    discs: settled.map((d) => ({ x: round(d.x), y: round(d.y), size: d.size })),
    gi: lo,
  }
}
...
function nudgeVoids(
  ...
  for (let pass = 0; pass < 10; pass++) {
    ...
    for (let i = nRim; i < cur.length; i++) {
      const dd =
        Math.hypot(nth(cur, i).x - wx, nth(cur, i).y - wy) -
        nth(cur, i).size / 2
      if (dd < nd) {
        nd = dd
        nearest = i
    ...
    nth(trial, nearest).x += (wx - nth(trial, nearest).x) * 0.45
    nth(trial, nearest).y += (wy - nth(trial, nearest).y) * 0.45
    const settled = relax(trial, nRim, R, gi)
    if (!settled) break
    const next = settled.map((d) => ({
      x: round(d.x),
      y: round(d.y),
      size: d.size,
    }))
...
function build(
  ...
  for (let k = 0; k < m; k++)
    rimIdx.add(
      Math.min(
        sizes.length - 1,
        Math.floor((k + phase / 2) * (sizes.length / m)),
      ),
    )
  if (rimIdx.size < m) return null
  const rimPool = [...rimIdx].map((i) => nth(sizes, i)).sort((a, b) => b - a)
  const rimOrder: number[] = []
  for (let k = 0; k < Math.ceil(m / 2); k++) {
    rimOrder.push(nth(rimPool, k))
    if (m - 1 - k > k) rimOrder.push(nth(rimPool, m - 1 - k))
  }
  const area =
    sizes.reduce((a, z) => a + (z / 2 + g / 2) ** 2, 0) +
    (pin > 0 ? (pin / 2 + g / 2) ** 2 : 0)
  const R = Math.sqrt(area / eta)
  ...
  // The pin: a seat holding the exact centre that never moves — it
  // joins the immovable prefix alongside the rim.
  if (pin > 0) discs.push({ x: 0, y: 0, size: pin })
  const fixed = m + (pin > 0 ? 1 : 0)
  ...
  while (interior.length > 0) {
    ...
    for (let at = 0; at < interior.length; at++) {
      if (seen.has(nth(interior, at))) continue
      seen.add(nth(interior, at))
      const spot = bestSpot(discs, nth(interior, at), g, R)
      if (spot) options.push({ at, ...spot })
    }
    if (options.length === 0) {
      if (retries-- <= 0) return null
      const squeezed = compact(discs, fixed, g)
      discs.length = 0
      discs.push(...squeezed)
      continue
    }
    ...
    const pick = nth(
      options
        ...
        .sort((a, b) => nth(interior, b.at) - nth(interior, a.at)),
      0,
    )
    discs.push({
      x: round(pick.x),
      y: round(pick.y),
      size: nth(interior, pick.at),
    })
    interior.splice(pick.at, 1)
  }
  return { discs, R, nRim: fixed }
}
...
function fitOf(discs: Disc[]) {
  ...
  for (const d of discs) {
    const sector =
      Math.floor(((Math.atan2(d.y, d.x) + Math.PI) / (Math.PI * 2)) * SECTORS) %
      SECTORS
    const rim = Math.hypot(d.x, d.y) + d.size / 2
    if (rim > nth(rims, sector)) rims[sector] = rim
  }
...
function voidsOf(discs: Disc[], R: number, g: number) {
  ...
  return {
    worst: wr * 2,
    spacing: Math.min(100, (idealPocket / Math.max(wr * 2, 0.01)) * 100),
  }
}
...
/* `pin` (a diameter in layout units) reserves the exact centre for a
   seat the solver may not move — the claim's profile disc. */
export function solveCrowd(n: number, pin = 0): Solved {
  const key = `${n}:${pin}`
    ...
    for (const eta of [0.9, 0.87, 0.84, 0.8, 0.76, 0.72, 0.68, 0.64]) {
      for (let m = mLo; m <= mHi; m++) {
        for (const phase of [0, 1]) {
          const built = build(sizes, m, phase, eta, g, pin)
          if (!built) continue
          ...
          if (candRound && !bestRound) best = candidate
            ...
            candRound === bestRound &&
            (candRound ? spacing > best.spacing : fit > best.fit)
          )
            best = candidate
        }
      }
  ...
  if (!winner) {
    ...
    return solveCrowd(
      Math.max(MIN_N, Math.min(MAX_N, n > DEFAULT_N ? n - 1 : n + 1)),
      pin,
    )
  }
...
function rosterFor(discs: Disc[]): string[] {
  const centreIdx = discs.reduce(
    (bi, d, i) =>
      Math.hypot(d.x, d.y) < Math.hypot(nth(discs, bi).x, nth(discs, bi).y)
        ? i
        : bi,
    0,
  )
...
export function WorldCrowd({
  ...
  const enterDelay = (i: number) =>
    `${Math.round((0.05 + (Math.hypot(nth(discs, i).x, nth(discs, i).y) / R) * 0.35) * 100) / 100}s`
  ...
  return (
    <div>
      <div className="relative mx-auto w-full" style={{ aspectRatio: '1 / 1' }}>
        ...
        {discs.map((d, i) => (
          <span
            key={`${count}-${nth(people, i)}-${i}`}
            className={`aw-enter aw-item absolute ${onPick ? 'cursor-pointer' : ''} ${
              selected === nth(people, i) ? 'aw-item--picked' : ''
            }`}
            onClick={onPick ? () => onPick(nth(people, i)) : undefined}
            style={{
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (Number.isFinite(v))
                setCount(Math.max(MIN_N, Math.min(MAX_N, Math.round(v))))
            }}
            type="number"
            value={count}
          />
