...
// The closing globe — the hive, given its instrument.
//
// A dome under a low camera, but the graticule is a bezel now: solid
// rings, dotted rings, and tick rings of two weights interleaved up
// the latitudes, the way a dial mixes its scales. Latitude rings are
// rotationally invariant — a spinning sphere of them looks still — so
// the ticks are what make the rotation legible: adjacent tick rings
// counter-rotate at their own paces, and a few solid arc fragments
// sweep their own latitudes just off the surface, each led by a small
// triangle marker, dimming as they swing behind the dome. The
// congregation still rides the rails — mostly bare dots now, a few
// scouts pulling comets — and Sara's apps ride among them as small
// chips. One blue, spent on the living.

import { useEffect, useRef } from "react";

// ── Stage ──────────────────────────────────────────────────────────────
// A wide dome: the camera sits low (shallow tilt), the sphere large,
// its lower half cropped by the page — a horizon.
const CX = 400;
const CY = 330;
const TILT = 0.32; // low camera — flat rings, the dome read
const S = 300; // sphere radius on screen
...
// The sphere's geometry still crowds them toward the pole; the roles
// keep any two neighbours from speaking in the same line.
const RING_LATS = Array.from({ length: 15 }, (_, i) => -32 + i * 8);

type RingRole = "solid" | "dot" | "minor" | "major";
const RING_ROLES: RingRole[] = [
  "solid", // -32 — the horizon course
  "minor", // -24
  "dot", //   -16
  "solid", //  -8
...
// rings stay empty; nobody orbits a puddle. Most riders are bare
// dots; every third is a scout and pulls the comet.
const N_RIDERS = 44;
const isScout = (i: number) => i % 3 === 0;
const N_RIDE_RAILS = 12; // lats -32°…56°
...
function posAt(i: number, t: number): P3 {
  ...
  const th = (RING_LATS[rd.rail] * Math.PI) / 180;
  const r = Math.cos(th);
  const y = Math.sin(th);
...
const LINK_O = (a: P3, b: P3) => {
  ...
  if (d > 0.28) return 0;
  return round(((0.28 - d) / 0.28) * 0.4 * (0.35 + 0.65 * ((a.nd + b.nd) / 2)));
};
...
export function FooterGlobe() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const heads: P3[] = [];
      for (let i = 0; i < N_RIDERS; i++) {
        const s = isScout(i) ? streakAt(i, t) : { d: "", head: posAt(i, t) };
        heads.push(s.head);
  ...
  return (
    <svg aria-hidden className="size-full" fill="none" viewBox="0 0 800 420">
      ...
      {/* trails — the scouts' actual last seconds, replayed */}
      {INIT.map((s, i) =>
        isScout(i) ? (
          <path
            key={`t${i}`}
