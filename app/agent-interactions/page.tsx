...
const SHIPPED_SECTIONS: Section[] = [
  {
    ...
    studies: [
      ...
      },
      {
        ...
        note: "Same red seal and jolt, but still afterwards — and once the seal settles, the bubble goes light: portrait to half ink, ring to 0.7, while the white disc stays fully opaque so stacked neighbors never bleed through. A stop was your own act — identity recedes hardest, the verdict stays legible. Failed dims nothing; the fade lifts quickly on a rerun.",
        demo: <ShippedStopped />,
      },
      {
      ...
      },
      {
        ...
        note: "The Gestures Depart (G6), promoted and tuned: dismissed, the bubble becomes weather — fade, 2.5px blur, 1.05 swell — drifting 1.5px rightward instead of up, out the row's own exit. The slot closes across the ENTIRE 1.6s, synchronized with the drift: the departing face rides its collapsing slot right while it evaporates, and the neighbors flow right with it from frame zero — transforms don't move layout, so the width moves WITH the mist, never after it. Tempo settled between the 680ms goodbye and the board loop's 2s; drift quartered from the study's 6px — the travel belongs to the slot ride. Release, together. (Dismissal is the opposite register, on purpose: a × is deletion, not departure — no direction, no travel, just a 150ms fade. Only agents leaving on their own leave rightward.)",
        demo: <ShippedDepart />,
      },
      {
        title: "Truncation",
