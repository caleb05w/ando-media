  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      if (win) {
        ...
        const recede = tc ? 1 - ease(seg(vt - tc.t, tc.hold, TYPE_EXIT)) : next == null ? 0 : easeInOut(seg(vt, next.t - next.lead, next.lead));
        win.style.opacity = `${Math.min(1, p * 1.6)}`;
        win.style.transform = `translateY(${20 * (1 - p)}px) scale(${1 - 0.05 * recede})`;
        win.style.filter = recede > 0 ? `blur(${3 * recede}px)` : "none";
      }
