...
// Shared channel chrome — the static shell of /channel (title bar, global
// rail, sidebar, page header) plus the read-only message row, lifted out so
// more than one prototype can sit inside a real-looking channel instead of
// floating on an empty page. Extracted verbatim from app/channel/page.tsx;
// the interactive parts (multi-select, agent presence, composer) stayed
// behind, since they are that page own behaviour.
...
export const A = "/multi-select";

export const FG_PRIMARY = "#1a1817";
export const FG_SECONDARY = "#58524e";
export const FG_TERTIARY = "#78716c";
export const STROKE_WEAK = "#f0efee";
export const BG_TERTIARY = "#f5f5f4";
export const BRAND = "#2563eb";

// The current user, for messages sent from a composer.
export const YOU: DemoAuthor = {
  id: "you",
  name: "You",
...
export function AgentSparkle({ cx, cy, scale }: { cx: number; cy: number; scale: number }) {
  return (
...
export function Silhouette({ size }: { size: number }) {
  return (
    <svg
      width={size}
      height={size}
...
export function SidebarRowView({ entry }: { entry: SidebarEntry }) {
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <div className="flex h-3 items-center gap-1.5 whitespace-nowrap">
          {/* Agent authors (hex-color avatars) carry their name in brand
              blue, per the suggestion flow — people stay ink. */}
          <span
            className="text-[14px] font-medium leading-5"
            style={{
              color: message.author.avatar.startsWith("#") ? BRAND : FG_PRIMARY,
            }}
          >
            {message.author.name}
          </span>
        ...
        <div
          data-msg-text
          // pre-wrap so a multi-line message (a staged draft, say) keeps its
          // line breaks at normal leading. Paragraph gaps stay the <p> mt-5;
          // this only affects newlines *inside* a paragraph's own text.
          ...
          {message.body.map((paragraph, pIndex) => (
            <p key={pIndex} className={pIndex > 0 ? "mt-5" : undefined}>
              {paragraph.map((segment, sIndex) =>
                segment.mention ? (
                  // @-mention pill: brand text on a brand tint, hugging the
                  // mention only — distinct from plain links, which stay
                  // bare blue text.
                  <span
                    key={sIndex}
...
export function ThreadFooter({ message }: { message: DemoMessage }) {
