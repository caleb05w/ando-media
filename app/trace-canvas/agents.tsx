...
// Agent engine + presence surfaces for /trace-canvas (blank-canvas fork of
...
export function CornerStack({
  ...
  resting?: boolean;
  /** Dynamic-Island reception: true while a demoted typing face is
      landing — the stack stretches toward the arrival, swallows, and
      jelly-settles (aw-tray-receive). */
  receiving?: boolean;
  /** Runs that arrived BY FLIGHT (demoted typers): their chips mount
      silently — the flying face already performed the arrival, so the
      dock slide-in would double it. */
  landedRunIds?: ReadonlySet<string>;
  ...
  useLayoutEffect(() => {
    ...
    for (const el of els) {
      ...
      if (prevLeft == null) {
        ...
        // flight-landed chips mount SILENTLY (the flying face already
        // performed the arrival); everything else is a DOCK — a bubble
        // arriving because its row scrolled away — and slides in from
        // the transcript's direction with a soft settle.
        if (
          !reduceMotion &&
          !el.classList.contains("aw-chip-in") &&
          !landedRunIds?.has(id)
        ) {
          el.animate(
            [
