/**
 * MultiSelect styles (duplicated from Select's geometry, not imported, so the
...
export const multiSelectTriggerStyles = [
...
// Base UI still sets data-placeholder while the selected array is empty.
export const multiSelectValueStyles = [
  'min-w-0 flex-1 truncate',
  'data-[placeholder]:text-text-tertiary',
]

// Trailing chevron: 24px from the right edge (px-6 inset) + icon-secondary tone.
export const multiSelectIconStyles = ['ml-2 flex shrink-0 text-icon-secondary']

// Per-option leading icon: the item row is already `flex items-center gap-2`,
// so it only needs to stay sized + muted (matches the trigger's value tone).
export const multiSelectItemIconStyles = ['flex shrink-0 text-icon-secondary']

// Trailing checkmark: Base UI's ItemIndicator only mounts for selected items,
// so the row layout (gap-2) handles spacing; tint it primary + keep it sized.
export const multiSelectItemIndicatorStyles = [
  'ml-2 flex shrink-0 text-icon-primary',
]

export const multiSelectPositionerStyles = ['z-[9999]']

export const multiSelectPopupStyles = [
  'flex flex-col',
...
export const multiSelectViewportStyles = [
...
export const multiSelectScrollbarStyles = [
